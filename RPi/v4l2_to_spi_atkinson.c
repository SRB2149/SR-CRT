...
{
    /* Work on a signed copy so error can go negative */
    int16_t buf[OUT_WIDTH * OUT_HEIGHT];
    for (int i = 0; i < OUT_WIDTH * OUT_HEIGHT; i++) buf[i] = scaled_frame[i];

    for (int y = 0; y < OUT_HEIGHT; y++) {
        for (int x = 0; x < OUT_WIDTH; x++) {
            int idx = y * OUT_WIDTH + x;
            int old = buf[idx];
            int new_pix = (old > 127) ? 255 : 0;
            buf[idx] = new_pix;
            int err = (old - new_pix) >> 3;  /* divide by 8 */

            /* Atkinson distributes 1/8 to each of 6 neighbours */
            if (x + 1 < OUT_WIDTH)                       buf[idx + 1] += err;
            if (x + 2 < OUT_WIDTH)                       buf[idx + 2] += err;
            if (x > 0 && y + 1 < OUT_HEIGHT)             buf[idx + OUT_WIDTH - 1] += err;
            if (y + 1 < OUT_HEIGHT)                      buf[idx + OUT_WIDTH] += err;
            if (x + 1 < OUT_WIDTH && y + 1 < OUT_HEIGHT) buf[idx + OUT_WIDTH + 1] += err;
            if (y + 2 < OUT_HEIGHT)                      buf[idx + 2 * OUT_WIDTH] += err;
        }
    }
    ...
    for (int y = 0; y < OUT_HEIGHT; y++) {
        for (int x = 0; x < OUT_WIDTH; x += 8) {
            uint8_t b = 0;
            for (int k = 0; k < 8; k++) {
                if (buf[y * OUT_WIDTH + x + k]) b |= (uint8_t)(0x80 >> k);
            }
            *out++ = b;
        }
    }
